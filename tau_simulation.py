...
minutes = [entry['minute'] for entry in tau.history]
...
ax1.plot(minutes, phospho_counts, color='tab:blue', label='Phosphorylation Count')
ax1.tick_params(axis='y', labelcolor='tab:blue')

ax2 = ax1.twinx()
ax2.set_ylabel('Aggregation State', color='tab:red')
ax2.plot(minutes, aggregation_numeric, color='tab:red', linestyle='--', label='Aggregation State')
...
def run_and_plot_simulation(env=None):
    import numpy as np
    from TauProtein import TauProtein
    import matplotlib.pyplot as plt
    ...
    print("phospho_counts:", phospho_counts)
    print("aggregation_states:", aggregation_states)
